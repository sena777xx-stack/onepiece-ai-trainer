...
import{runAiTurn}from'./ai-engine.js?v=3466';import{recordSelfPlayMatch}from'./ai-telemetry.js?v=3511';
...
export async function runSelfPlay(cards,deckLeft,deckRight,games=100,onProgress=()=>{}){
  const count=Math.max(1,Math.min(10000,Number(games)||100));
  const result={games:count,leftWins:0,rightWins:0,draws:0,stalls:0,totalTurns:0,startedAt:Date.now()};
  for(let game=0;game<count;game++){
    const engine=new GameEngine(cards,{player:deckLeft,ai:deckRight}),first=game%2===0?'player':'ai';
    engine.start(first);
