...
import{runAiTurn}from'./ai-engine.js?v=3988';import{recordSelfPlayMatch}from'./ai-telemetry.js?v=3513';
...
export async function runSelfPlay(cards,deckLeft,deckRight,games=100,onProgress=()=>{},firstMode='alternate'){
  ...
  for(let game=0;game<count;game++){
    ...
    if(!engine.state.winner||engine.state.turn>=18||left.don.active>=4||right.don.active>=4)result.issueSamples.push({game:game+1,winner:engine.state.winner||'none',turn:engine.state.turn,actions,phase:engine.state.phase,activeSide:engine.state.activeSide,pendingKind:engine.state.pending?.kind||null,pendingStage:engine.state.pending?.stage||null,leftDon:left.don.active,rightDon:right.don.active,leftLife:left.life.length,rightLife:right.life.length,leftHand:left.hand.length,rightHand:right.hand.length});
    result.games=game+1;
